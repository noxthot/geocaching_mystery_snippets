...
def choose_directory():
    ...
    questions = [
        inquirer.List("directory", message="Choose the input directory:", choices=dirs)
    ]
...
with open(
    os.path.join(selected_directory, "input_tomatch.txt"), "r", encoding="utf-8"
) as f:
    lines_tomatch = f.readlines()

with open(
    os.path.join(selected_directory, "input_dict.txt"), "r", encoding="utf-8"
) as f:
...
for idxToMatch, wordToMatch in enumerate(lines_tomatch_mod):
    ...
    for idxDict, wordDictMod in enumerate(lines_dict_mod):
        if sorted(wordToMatch) == sorted(wordDictMod):
            ...
            print(
                f"[MATCH] {lines_tomatch[idxToMatch]} -> {wordToMatch} -> {wordDictMod} -> {lines_dict[idxDict]}"
            )
            matchFound = True
            break
